...
Cell_qc<-function(expression_profile,sample_information=NULL,species="Hs",gene_low=500,gene_high=10000,mito_high=0.1,umi_low=1500,umi_high=Inf,logNormalize=TRUE,plot=FALSE,plot_path="./quality_control.pdf"){
  require(stringr)
  if(species=="Hs"){
  ...
  if(is.null(sample_information)){
    ...
    sample_information_qc<-sample_information[colnames(SQ_data_qc)]
    return(list("expression_profile"=SQ_data_qc,"sample_information"=sample_information_qc))
  }
}

### filter out cell type without the minimal cell number
Cell_type_filter<-function(expression_profile,sample_information,min_cell_number=10){
  ...
  return(list("expression_profile"=expression_profile_filtered,"sample_information"=sample_information_filtered))
}
